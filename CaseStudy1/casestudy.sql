...
SELECT 'movie' AS table_name, COUNT(*) AS row_count
FROM imdb.movie
UNION
SELECT 'genre' AS table_name, COUNT(*) AS row_count
FROM imdb.genre
UNION
SELECT 'director_mapping' AS table_name, COUNT(*) AS row_count
FROM imdb.movie
UNION
SELECT 'names' AS table_name, COUNT(*) AS row_count
FROM imdb.genre
UNION
SELECT 'ratings' AS table_name, COUNT(*) AS row_count
FROM imdb.movie
UNION
SELECT 'role_mapping' AS table_name, COUNT(*) AS row_count
FROM imdb.genre
;
...
-- There are 4 columns which consists of null values in the Movie Table ,They are  COUNTRY,worlwide_gross_income,languages,production_company
...
-- 887  movies were produced in the USA or India in the year 2019.
...
SELECT G.GENRE ,COUNT(*) AS NUM_OF_MOVIES
FROM MOVIE M 
...
WHERE M.country = 'USA'
    ...
    AND DATE_FORMAT(M.date_published, '%Y-%m') = '2017-03'
GROUP BY G.genre
...
SELECT
    ...
    AVG(AVG(M.duration)) OVER (PARTITION BY G.genre ORDER BY M.year) AS moving_avg_duration
FROM movie M
JOIN genre G ON M.id = G.movie_id
...
WITH MultilingualHits AS (
    ...
    FROM movie M
    JOIN ratings R ON M.id = R.movie_id
    WHERE POSITION(',' IN M.languages) > 0  -- Movies with more than one language
        AND R.median_rating >= 8 AND M.production_company IS NOT NULL
...
WITH DramaActresses AS (
    ...
    JOIN genre G ON M.id = G.movie_id
    JOIN ratings R ON M.id = R.movie_id
    WHERE G.genre = 'drama'  -- Drama genre
...
-- Q29. Details for top 9 directors based on the number of movies.
...
SELECT
    ...
    director_name,
    number_of_movies,
    ROUND(avg_inter_movie_duration, 2) AS avg_inter_movie_days,
